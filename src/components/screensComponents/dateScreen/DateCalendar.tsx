...
const DateCalendar = ({sel, setSel, dateSlots}: IDateCalendar) => {
  ...
  const circleColor = (date: Date): string => {
    return sel.valueOf() - date.valueOf() == 0
      ? 'selected'
      : startOfDay(new Date()).valueOf() - date.valueOf() == 0
      ? 'start'
      : 'normal';
  };
  ...
  const renderWeek = (item: weekDay) => (
    <View style={{width: (width - 30) / 7}} key={item.key}>
      <Text
        style={[
          styles.dayText,
          {color: circleColor(item.date) == 'selected' ? '#373737' : '#959595'},
        ]}>
        {Day(item.key)}
      </Text>
      <Pressable
        style={styles.dayNumContainer}
        onPress={() =>
          startOfDay(new Date()).valueOf() - item.date.valueOf() <= 0
            ? setSel(item.date)
            : null
        }>
        <Text
          style={[
            globalStyles.H3,
            styles.normalCircle,
            circleColor(item.date) == 'selected' ? styles.selCircle : {},
